...
const DockItem: React.FC<DockItemProps> = ({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="relative inline-flex items-center justify-center rounded-full bg-gray-700/80 shadow-lg"
      tabIndex={0}
      role="button"
      aria-haspopup="true"
    >
      ...
      <AnimatePresence>
        {showLabel && (
          <motion.div
            ...
            className="absolute -top-6 left-1/2 w-fit whitespace-pre rounded-md border border-gray-600 bg-gray-900 px-2 py-0.5 text-xs text-white"
            style={{ x: "-50%" }}
            role="tooltip"
          >
            {label}
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`fixed bottom-2 left-1/2 -translate-x-1/2 transform 
            flex items-end gap-4 w-fit rounded-2xl 
            border px-4 pb-2 ${className}`}
