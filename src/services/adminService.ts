...
// Schema: id, title, description, room, priority, patient_id, assigned_to, status, created_at
const mapTask = (data: any): Task => ({
  id: data.id,
  title: data.title,
  description: data.description || `Room: ${data.room || 'N/A'}`,
...
export const updateTask = async (id: string, task: Partial<Task>) => {
    ...
    if (task.assignedTo !== undefined) payload.assigned_to = task.assignedTo;
